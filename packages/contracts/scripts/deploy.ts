...
async function main() {
  ...
  const response = await wasmd.query(
    "orai167r4ut7avvgpp3rlzksz6vw5spmykluzagvmj3ht845fjschwugqjsqhst",
    req
  );
  const pairResponseFactory = JSON.parse(
    Buffer.from(response.slice(2), "hex").toString("utf-8")
  ) as any;
