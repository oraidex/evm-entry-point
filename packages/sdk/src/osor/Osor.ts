...
export class Osor {
  ...
  ORAICHAIN_OSOR_ROUTER_ADDRESS =
    'orai1yglsm0u2x3xmct9kq3lxa654cshaxj9j5d9rw5enemkkkdjgzj7sr3gwt0';

  constructor(private readonly osorUrl: string) {
  ...
  async getSwapOraidexMsg(
    ...
    try {
      ...
      const miniumAmount = new Decimal(route.returnAmount).mul(
        new Decimal(100).sub(slippageTolerance || 0).div(100),
      );
      const msgs = route.routes.map((route) =>
        this.osorMsgComposer.generateMsgFromRouteResponse(route),
      );
